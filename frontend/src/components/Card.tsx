...
function CardRoot({ children, className, props }: CardProps) {
  return (
    <div className={cx('border border-zinc-800 rounded-md', className)} {...props}>
      {children}
    </div>
  )
}
...
function CardHeader({ children, className, props, title, subtext }: CardProps & {
  ...
  return (
    <div className={cx('flex justify-between items-center p-4 border-b border-zinc-800', className)} {...props}>
      <div className="flex flex-col items-start">
        <h2 className="text-zinc-50 font-medium leading-6 text-[15px] tracking-[0.15px]">{ title }</h2>
        <span className="text-zinc-400 text-[14px] tracking-[0.1px] leading-4">{ subtext }</span>
      </div>
      {children}
    </div>
  )
